...
// Simple logger
function log(...args: any[]) { if ((process.env.LOG_LEVEL || 'info') !== 'silent') console.log(...args); }
...
app.post('/api/check-electricity/bulk', requireAuth, async (req, res) => {
  try {
    const contract_numbers = Array.isArray(req.body.contract_numbers) ? req.body.contract_numbers.map((v: any) => String(v)).map((s: string) => s.trim()).filter(Boolean) : [];
    const sku = (req.body.sku || req.body.provider_id || '').toString().trim();
