...
async function supabaseCreate(member) {
    const base = SUPABASE_URL.replace(/\/$/, '') + '/rest/v1/members';
    const headers = {
        apikey: SUPABASE_SERVICE_ROLE,
        Authorization: `Bearer ${SUPABASE_SERVICE_ROLE}`,
...
async function supabaseUpdate(id, values) {
...
// In-memory helpers
function memGetAll() {
    return Object.values(MEM.MEMBERS || {}).sort((a, b) => (b.created_at || '').localeCompare(a.created_at || ''));
}
function memGetOne(id) {
    return MEM.MEMBERS ? MEM.MEMBERS[String(id)] || null : null;
}
function memCreate(member) {
...
function memUpdate(id, values) {
    if (!MEM.MEMBERS || !MEM.MEMBERS[id])
        return null;
...
function validateMemberInput(payload) {
    ...
    if (payload.name.length > 200)
        return 'Tên quá dài';
    if (payload.zalo && String(payload.zalo).length > 50)
        return 'Zalo quá dài';
    if (payload.bank && String(payload.bank).length > 200)
        return 'Bank quá dài';
    return null;
}
// Handler
const handler = async (event) => {
    try {
        ...
        if (method === 'GET') {
            ...
            try {
                if (SUPABASE_URL && SUPABASE_SERVICE_ROLE) {
                    ...
                    if (id)
                        return jsonResponse(200, Array.isArray(data) && data.length ? normalizeMemberRow(data[0]) : null);
                    return jsonResponse(200, (data || []).map(normalizeMemberRow));
                }
                else {
                    if (id)
                        return jsonResponse(200, memGetOne(String(id)));
        ...
        if (method === 'POST') {
            ...
            if (v)
                return jsonResponse(400, { error: v });
            if (SUPABASE_URL && SUPABASE_SERVICE_ROLE) {
                try {
        ...
        if (method === 'PUT' || method === 'PATCH') {
            ...
            if (body.name !== undefined) {
                if (typeof body.name !== 'string' || body.name.trim().length === 0)
                    return jsonResponse(400, { error: 'Tên không hợp lệ' });
                if (body.name.length > 200)
                    return jsonResponse(400, { error: 'Tên quá dài' });
            }
