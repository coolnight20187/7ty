...
function logDebug(...args) { if (LOG_LEVEL === 'debug')
    console.debug('[kho-list]', ...args); }
function logInfo(...args) { if (['debug', 'info'].includes(LOG_LEVEL))
    console.info('[kho-list]', ...args); }
function logWarn(...args) { if (['debug', 'info', 'warn'].includes(LOG_LEVEL))
    console.warn('[kho-list]', ...args); }
function logError(...args) { console.error('[kho-list]', ...args); }
// In-memory store (shared across function instances only while warm)
let IN_MEMORY_STORE = global.__KHO_MEMORY_STORE__ || {};
global.__KHO_MEMORY_STORE__ = IN_MEMORY_STORE;
...
async function querySupabase(params) {
    ...
    if (params.fromAmount != null)
        filters.push(`total=gte.${Number(params.fromAmount)}`);
    if (params.toAmount != null)
        filters.push(`total=lte.${Number(params.toAmount)}`);
    if (params.provider_id)
        filters.push(`provider_id=eq.${encodeURIComponent(params.provider_id)}`);
    if (params.search) {
    ...
    if (filters.length)
        qs.set('q', filters.join('&')); // Note: PostgREST expects each filter as separate query param; we'll append manually below
    ...
    if (!url.searchParams.has('order'))
        url.searchParams.set('order', 'nhapAt.desc');
...
function queryMemory(params) {
    const arr = Object.values(IN_MEMORY_STORE || {});
    let res = arr.slice();
    if (params.provider_id) {
        res = res.filter((r) => String(r.provider_id) === String(params.provider_id));
    }
    if (params.fromAmount != null) {
        res = res.filter((r) => Number(r.total || r.amount_current || 0) >= Number(params.fromAmount));
    }
    if (params.toAmount != null) {
        res = res.filter((r) => Number(r.total || r.amount_current || 0) <= Number(params.toAmount));
    }
    if (params.search) {
        const s = String(params.search).toLowerCase();
        res = res.filter((r) => {
...
export { handler };
//# sourceMappingURL=kho-list.js.map