...
function logDebug(...args) { if (LOG_LEVEL === 'debug')
    console.debug('[kho-import]', ...args); }
function logInfo(...args) { if (['debug', 'info'].includes(LOG_LEVEL))
    console.info('[kho-import]', ...args); }
function logWarn(...args) { if (['debug', 'info', 'warn'].includes(LOG_LEVEL))
    console.warn('[kho-import]', ...args); }
function logError(...args) { console.error('[kho-import]', ...args); }
let IN_MEMORY_STORE = {};
// Basic helpers
function isValidBill(b) {
...
async function upsertSupabase(bills) {
...
async function upsertInMemory(bills) {
    let added = 0;
    let updated = 0;
...
const handler = async (event) => {
    try {
        ...
        if (!valid.length)
            return { statusCode: 400, body: JSON.stringify({ error: 'No valid bills found' }) };
...
export { handler };
//# sourceMappingURL=kho-import.js.map